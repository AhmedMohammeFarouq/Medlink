...
const doctorId = doctor ? doctor._id : req.user.userId;
  ...
  static async getAll(req, res, next) {
    try {
      ...
      if (req.user?.role === "DOCTOR") {
        const doctor = await Doctor.findOne({ userId: req.user.userId });
        if (doctor) {
          filter = { $or: [{ doctorId: doctor._id }, { doctorId: req.user.userId }] };
        } else {
      ...
      const prescriptions = await PrescriptionService.getAllPrescriptions({ ...filter, ...req.query });
