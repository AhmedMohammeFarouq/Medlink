...
export class AdminSettingsComponent implements OnInit {
  ...
  revokeSession(session: UserSession): void {
    this.confirmTitle = 'Revoke Administrator Session';
    this.confirmMessage = `Disconnect administrator session (${session.deviceInfo || session.ipAddress || 'Device'})?`;
    this.confirmAction = () => {
      this.userService.revokeCurrentUserSession(session._id).subscribe({
        next: () => {
          this.isConfirmOpen = false;
          this.successMessage = 'Session revoked.';
          this.loadSessions();
  ...
  revokeAll(): void {
    this.confirmTitle = 'Disconnect All Sessions';
    this.confirmMessage = 'Revoke all administrator sessions across all devices?';
    this.confirmAction = () => {
      this.userService.revokeAllCurrentUserSessions().subscribe({
        next: () => {
          this.isConfirmOpen = false;
          this.successMessage = 'All administrator sessions revoked.';
          this.loadSessions();
          setTimeout(() => this.successMessage = null, 3000);
        },
        error: (err) => {
